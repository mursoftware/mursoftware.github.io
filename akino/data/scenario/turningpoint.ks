[_tb_system_call storage=system/_turningpoint.ks]

*スタート

[playbgm  volume="50"  time="1000"  loop="true"  storage="VSQSE_1121_mountain_02.mp3"  fadein="true"  ]
[bg  time="1000"  method="crossfade"  storage="夕焼け.jpg"  ]
[mask_off  time="1000"  effect="fadeOut"  ]
[playse  volume="100"  time="1000"  buf="0"  storage="革靴で歩く_自分.mp3"  loop="true"  fadein="true"  ]
[chara_show  name="AI"  time="1000"  wait="true"  storage="chara/1/後_無表情_暗.png"  width="332"  height="665"  left="460"  top="52"  reflect="false"  ]
[wait  time="10000"  ]
[stopbgm  time="3000"  fadeout="true"  ]
[bg  time="3000"  method="crossfade"  storage="黒.jpg"  ]
[stopse  time="1000"  buf="0"  ]
[wait  time="3000"  ]
[chara_hide  name="AI"  time="0"  wait="false"  pos_mode="false"  ]
[bg  time="0"  method="crossfade"  storage="横断歩道.jpg"  ]
[playse  volume="100"  time="1000"  buf="0"  storage="noise.mp3"  ]
[wait  time="200"  ]
[chara_show  name="AI"  time="0"  wait="false"  storage="chara/1/後_無表情_暗.png"  width="332"  height="665"  left="460"  top="52"  reflect="false"  ]
[bg  time="0"  method="crossfade"  storage="黒.jpg"  ]
[wait  time="3000"  ]
[tb_show_message_window  ]
[stopse  time="1000"  buf="0"  ]
[chara_mod  name="AI"  time="600"  cross="true"  storage="chara/1/後_無表情.png"  ]
[tb_start_text mode=1 ]
ねぇ[p]
＜運命＞ってやつ[p]
信じてる？[p]
[_tb_end_text]

[chara_mod  name="AI"  time="600"  cross="true"  storage="chara/1/振返り_怒る.png"  ]
[tb_start_text mode=1 ]
まあ、あたしはプログラムだからさ[p]
プログラムされた＜運命＞のとおりにしか動けないんだけどさ・・・[p]
[_tb_end_text]

[chara_mod  name="AI"  time="600"  cross="true"  storage="chara/1/後_無表情.png"  ]
[tb_start_text mode=1 ]
コンピュータの性能が上がって、天気予報の精度が上がっているじゃない？[p]
この先、コンピュータの性能がどんどん上がって[p]
あらゆる情報をインプット出来たら、[p]
全ての現象の予測が可能になるってことじゃない？[p]
つまりそれって、「全ての現象は予め決まっている」ってことじゃない？[p]
あんたも、このゲームの選択肢を[p]
自分の意志で選んでいるつもりかもしれないけど、[p]
その意思もあらかじめ決まっていた自然現象の一環なんじゃないの？[p]
・・・[p]
・・・・・・[p]
[_tb_end_text]

[playbgm  volume="80"  time="1000"  loop="true"  storage="VSQSE_0035_city_noise.mp3"  fadein="true"  ]
[bg  time="1000"  method="crossfade"  storage="交差点.jpg"  ]
[chara_mod  name="AI"  time="600"  cross="true"  storage="chara/1/振返り_笑顔.png"  ]
[tb_start_text mode=1 ]
ここでお別れね[p]
なんだかんだで[p]
意外と楽しかったよ[p]
ありがとう[p]
じゃあ・・・[p]
あたしは＜左＞、あんたは＜右＞、ね[p]
[_tb_end_text]

[glink  color="black"  storage="turningpoint.ks"  size="20"  text="逆じゃない？"  target="*逆"  x="450"  y="300"  width="300"  ]
[glink  color="black"  storage="turningpoint.ks"  size="20"  text="うちは左だけど・・・"  target="*逆"  x="450"  y="400"  width="300"  ]
[stopbgm  time="1000"  fadeout="true"  ]
[s  ]
*逆

[chara_mod  name="AI"  time="600"  cross="true"  storage="chara/1/振返り_怒る.png"  ]
[tb_start_text mode=1 ]
あたしの言うことに逆らうつもり！？[p]
あたしは＜左＞、あんたは＜右＞[p]
分かった！？[p]
[_tb_end_text]

[glink  color="black"  storage="turningpoint.ks"  size="20"  text="でも・・・"  target="*左"  x="450"  y="300"  width="300"  ]
[glink  color="black"  storage="turningpoint.ks"  size="20"  text="うちは左だって！"  target="*左"  x="450"  y="400"  width="300"  ]
[s  ]
*左

[chara_mod  name="AI"  time="600"  cross="true"  storage="chara/1/後_無表情.png"  ]
[tb_start_text mode=1 ]
・・・[p]
・・・お願いだから、あたしの言うことを聞いて！・・・[p]
・・・お願い・・・[p]
・・・[p]
じゃあ　さよなら！[p]
[_tb_end_text]

[tb_hide_message_window  ]
[playse  volume="100"  time="1000"  buf="0"  storage="アスファルトの上を歩く2.mp3"  ]
[chara_hide  name="AI"  time="1000"  wait="true"  pos_mode="true"  ]
[stopse  time="3000"  buf="0"  fadeout="true"  ]
[wait  time="5000"  ]
[stopbgm  time="3000"  fadeout="true"  ]
[mask  time="3000"  effect="fadeIn"  color="0x000000"  ]
[bg  time="1000"  method="crossfade"  storage="住宅街2.jpg"  ]
[playbgm  volume="100"  time="1000"  loop="true"  storage="VSQSE_0035_city_noise.mp3"  fadein="true"  ]
[playse  volume="100"  time="1000"  buf="0"  storage="革靴で歩く_自分.mp3"  fadein="true"  loop="true"  ]
[mask_off  time="1000"  effect="fadeOut"  ]
[wait  time="8000"  ]
[stopbgm  time="1000"  fadeout="true"  ]
[stopse  time="1000"  buf="0"  ]
[wait  time="2000"  ]
[glink  color="black"  storage="turningpoint.ks"  size="20"  text="進む"  x="450"  y="300"  width="300"  target="*進む"  ]
[glink  color="black"  storage="turningpoint.ks"  size="20"  text="戻る"  x="450"  y="400"  width="300"  target="*戻る"  ]
[s  ]
*進む

[playse  volume="100"  time="1000"  buf="0"  storage="革靴で歩く_自分.mp3"  ]
[wait  time="3000"  ]
[stopse  time="1000"  buf="0"  fadeout="true"  ]
[stopbgm  time="1000"  fadeout="true"  ]
[mask  time="3000"  effect="fadeIn"  color="0x000000"  ]
[wait  time="3000"  ]
[bg  time="0"  method="crossfade"  storage="黒.jpg"  ]
[mask_off  time="0"  effect="fadeOut"  ]
[bg  time="0"  method="crossfade"  storage="横断歩道_靴.jpg"  ]
[wait  time="2000"  ]
[bg  time="0"  method="crossfade"  storage="黒.jpg"  ]
[wait  time="3000"  ]
[iscript]
alert( "キャラクターデータが削除されました" );
[endscript]

[wait  time="3000"  ]
[jump  storage="title_screen3.ks"  target="*スタート"  ]
*戻る

[playse  volume="200"  time="1000"  buf="0"  storage="アスファルトの上を歩く2.mp3"  ]
[wait  time="3000"  ]
[stopbgm  time="1000"  fadeout="true"  ]
[stopse  time="1000"  buf="0"  fadeout="true"  ]
[mask  time="0"  effect="fadeIn"  color="0x000000"  ]
[jump  storage="crash.ks"  target="*スタート"  ]
